...
# -- GPL V 3.0 license.
# ------------------------------------------------------------------------------
# -- Modified Dec 2017 by Jeremy Bundgaard
# -- Modified run folders for stable batch processing many timeCal simultaneously
# ------------------------------------------------------------------------------


speedOfLight=29.9792458 #cm/ns
timecalInstallDir=/SCRATCH/DRunScratch1/ornl2016/Rb/configs/timeCal
...
then
    ...
    if [[ "$#" < 2 ]]; then
      resultDir=$PWD/$hisbase
    fi
    echo -e "Creating Result directory: $resultDir"
    mkdir -p $resultDir
    cd $resultDir
...
then
    SetParams "small"
    OutputInfo
    CalculateAndOutput > unformatted_smallConfig.xml
    xmllint --format unformatted_smallConfig.xml > smallConfig.xml
    rm unformatted_smallConfig.xml
fi
...
then
    SetParams "big"
    OutputInfo
    CalculateAndOutput > unformatted_bigConfig.xml
    xmllint --format unformatted_bigConfig.xml > bigConfig.xml
    rm unformatted_bigConfig.xml
fi
...
then
    SetParams "medium"
    OutputInfo
    CalculateAndOutput > unformatted_mediumConfig.xml
    xmllint --format unformatted_mediumConfig.xml > mediumConfig.xml
    rm unformatted_mediumConfig.xml
fi
...
mkdir debug;mv {fit.log,$tmpDir} debug/.
cd - > /dev/null 2>&1
