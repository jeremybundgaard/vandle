...
plotdir=`pwd`/eventDisplay_`date +%d%b%y_%H%M.%S`
mkdir $plotdir $plotdir/plots
cd $plotdir
cp $macros/eventDisplay.{C,sh} .

echo "root -l -q -b eventDisplay.C\+\+\(\"$data_summary_tree\",\"$cuts\")"
time  root -l -q -b eventDisplay.C\+\+\(\"$data_summary_tree\",\"$cuts\"\)

rm eventDisplay_*{.d,so,pcm}
