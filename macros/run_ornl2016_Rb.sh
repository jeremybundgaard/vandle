...
#
# if [[ ! -f $config ]]; then
# 	echo "config $config doesn't exist"
# 	exit 97
# fi
#
# [[ "1" = "$dryrun" ]] && exit
...
# start=$SECONDS
# nice -n 19 $paassDir/install/bin/utkscan -i $infile -c $config -o $ldf $batch >> stdout.log
...
# cd - > /dev/null 2>&1
# mv $outdir /SCRATCH/DScratch3/jeremy/utkscan_output/
# rm $lockfile
