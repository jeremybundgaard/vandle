...
double minTime=211.20e12;
double maxTime=211.28e12;
int plotBins=1000;

TH1D* GeTime_hist  = new TH1D("GeTime_hist","GeTime",plotBins,minTime,maxTime);
GeTime_hist->SetLineColor(kViolet);
...
TH1D* vandle_TAvg_hist  = new TH1D("vandle_TAvg_hist","vandle_TAvg",plotBins,minTime,maxTime);
vandle_TAvg_hist->SetLineColor(kRed);
vandle_tree->Draw("vandle_TAvg>>vandle_TAvg_hist");
hs->Add(vandle_TAvg_hist);legend->AddEntry(vandle_TAvg_hist);
c1->SaveAs("vandle_TAvg_hist.png");


TH1D* vandle_TAvg_NEUTRON_tcutg_hist  = new TH1D("vandle_TAvg_NEUTRON_tcutg_hist","vandle_TAvg_NEUTRON_tcutg_hist",plotBins,minTime,maxTime);
vandle_TAvg_hist->SetLineColor(kGreen);
vandle_tree->Draw("vandle_TAvg>>vandle_TAvg_NEUTRON_tcutg_hist","tcutg_NEUTRON","");
hs->Add(vandle_TAvg_NEUTRON_tcutg_hist);legend->AddEntry(vandle_TAvg_NEUTRON_tcutg_hist);
c1->SaveAs("vandle_TAvg_NEUTRON_tcutg_hist.png");
...
for (int i = 0; i < vandle_tree->GetEntries(); i++) {
  ...
  if(tcutg_NEUTRON->IsInside(vandle_TOF,vandle_QDC)==1){
    // printf(" beta_TAvg:%.7f \n",beta_TAvg);
    beta_neutron_time_set.insert(beta_TAvg);
  }
}
...
neutronGated_HPGe_tree->Branch("aux_GeEn",&aux_GeEn);
neutronGated_HPGe_tree->Branch("aux_GeNum",&aux_GeNum);
neutronGated_HPGe_tree->Branch("aux_GeTime",&aux_GeTime);
neutronGated_HPGe_tree->Branch("aux_GeTdiff",&aux_GeTdiff);
neutronGated_HPGe_tree->Branch("aux_gMulti",&aux_GeMulti);
neutronGated_HPGe_tree->Branch("aux_GeHasLRBeta",&aux_GeHasLRBeta);
neutronGated_HPGe_tree->Branch("aux_BetaEn",&aux_BetaEn);
neutronGated_HPGe_tree->Branch("aux_BetaTime",&aux_BetaTime);
neutronGated_HPGe_tree->Branch("aux_BetaMulti",&aux_BetaMulti);
neutronGated_HPGe_tree->Branch("aux_cycle",&aux_cycle);
neutronGated_HPGe_tree->Branch("aux_cycleSTime",&aux_cycleSTime);
neutronGated_HPGe_tree->Branch("aux_eventNum",&aux_eventNum);
...
printf("vandle_TAvg_NEUTRON_tcutg_hist->Integral():%f\n",vandle_TAvg_NEUTRON_tcutg_hist->Integral());
