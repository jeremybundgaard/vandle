...
void pidCuts_stdDev(int preamp,const char* treeFile,const char* cuts="&&1") {
	...
	TCutG *alphaCut[nStdDev];
	for (int i = minStdDev; i < nStdDev; ++i)
	{
		sprintf(dumChar,"alphaCut_preamp%d_%dstdDev",preamp,i);
		TCutG* alphaCut_tmp= new TCutG(dumChar,10);
		alphaCut_tmp->SetVarX("adc");
		alphaCut_tmp->SetVarY("length");
		alphaCut_tmp->SetLineColor(kGreen);
		alphaCut_tmp->SetLineWidth(2);
		alphaCut_tmp->SetFillColorAlpha(kGreen,0.15);
		int pointNum=0;
		alphaCut_tmp->SetPoint(pointNum,alpha_minCutEnergy,alpha_minCutLength);pointNum++;
		alphaCut_tmp->SetPoint(pointNum,alphaFit_plusStdDev[i]->GetX(alpha_minCutLength),alpha_minCutLength);pointNum++;
		for (double energy = alphaFit_plusStdDev[i]->GetX(alpha_minCutLength); energy < 20; energy+=tcutStep){
			...
			alphaCut_tmp->SetPoint(pointNum,energy,alphaFit_plusStdDev[i]->Eval(energy));pointNum++;
		}
		for (double energy = 8.5; energy > alpha_minCutEnergy; energy-=tcutStep){
			...
			alphaCut_tmp->SetPoint(pointNum,energy,alphaFit_minusStdDev[i]->Eval(energy));pointNum++;
		}
		alphaCut_tmp->SetPoint(pointNum,alpha_minCutEnergy,alpha_minCutLength);
		alphaCut[i] = alphaCut_tmp;

		alphaCut[i]->GetXaxis()->SetRangeUser(0,8);
		alphaCut[i]->GetYaxis()->SetRangeUser(0,8);
		alphaCut[i]->Draw();
		sprintf(dumChar,"alphaCut_stdDev%d.png",i);
		c1->SaveAs(dumChar);

		sprintf(dumChar,"alphaCut_preamp%d_%dstdDev.C",preamp,i);
		ofstream alphaPrimitive;
		alphaPrimitive.open(dumChar);
		alphaCut[i]->SavePrimitive(alphaPrimitive);
	...
	for (int energyBin = energyBin_start; energyBin < energyBin_stop; energyBin+=energyBinStep){
		...
		energyCutPoint_frags_plusStdDev[arrayPos] = energyBin*enCal;
	...
	// 		double energy_plus=energyBin*enCal;
	// 		TLine *Line_plusStdDev = new TLine(
	// 			energy_plus,length-0.02,
	// 			energy_plus,length+0.02);
	// 		Line_plusStdDev->SetLineColor(i);
	...
	////////////////////////////////////////////////////////////////////////////////
	printf("\n\n/////////////////////// creating frag TCutG  ///////////////////\n\n");
	TCutG *fragCut[nStdDev];
	TF1* frag_lengthline = new TF1("frag_lengthline","[1]*x+[0]",0,100);
	frag_lengthline->SetLineWidth(5);
	frag_lengthline->SetParameter(0,1.1);
	frag_lengthline->SetParameter(1,0.04);
	c1->SetLogz(1);
	c1->Close();
	...
	for (int i = minStdDev; i < nStdDev; ++i)
	{
		sprintf(dumChar,"fragCut_preamp%d_%dstdDev",preamp,i);
		TCutG *fragCut_tmp = new TCutG(dumChar,4);
		fragCut_tmp->SetVarX("adc");
		fragCut_tmp->SetVarY("length");
		fragCut_tmp->SetLineColor(kViolet);
		fragCut_tmp->SetLineWidth(1);
		fragCut_tmp->SetFillColorAlpha(kViolet,0.15);
		int pointNum=0;
		fragCut_tmp->SetPoint(pointNum,150,alpha_minCutLength);pointNum++;
		fragCut_tmp->SetPoint(pointNum,alphaFit_plusStdDev[i]->GetX(alpha_minCutLength),alpha_minCutLength);pointNum++;
		double lastEnergy=0;
		for (double energy = alphaFit_plusStdDev[i]->GetX(alpha_minCutLength); energy < 4; energy+=tcutStep){
			...
			if (alphaFit_plusStdDev[i]->Eval(energy)>frag_lengthline->Eval(energy)){
				fragCut_tmp->SetPoint(pointNum,energy,frag_lengthline->Eval(energy));pointNum++;
			}
			else {
				fragCut_tmp->SetPoint(pointNum,energy,alphaFit_plusStdDev[i]->Eval(energy));pointNum++;
			}

			lastEnergy=energy;
		}
		fragCut_tmp->SetPoint(pointNum,lastEnergy,frag_lengthline->Eval(lastEnergy));pointNum++;
		fragCut_tmp->SetPoint(pointNum,30,frag_lengthline->Eval(30));pointNum++;
		fragCut_tmp->SetPoint(pointNum,71,3.15);pointNum++;
		fragCut_tmp->SetPoint(pointNum,150,3.65);pointNum++;
		fragCut_tmp->SetPoint(pointNum,150,alpha_minCutLength);pointNum++;
		fragCut[i] = fragCut_tmp;
		sprintf(dumChar,"fragCut_preamp%d_%dstdDev.C",preamp,i);
		ofstream fragPrimitive;
		fragPrimitive.open(dumChar);
		fragCut[i]->SavePrimitive(fragPrimitive);
	...
	for (int i = minStdDev; i < nStdDev; ++i){
		...
		for (int energyBin = 0; energyBin < 4000; ++energyBin){
			for (int lengthBin = 0; lengthBin < 1000; ++lengthBin){
				...
				if (alphaCut[i]->IsInside(energy,length))
					noMaskLvEn_alphas_tmp->SetBinContent(energyBin,lengthBin,counts);
				if (fragCut[i]->IsInside(energy,length))
	...
	for (int i = minStdDev; i < nStdDev; ++i)
	{
		...
		TH2D* noMaskLvEn_alphas_meanShifted_tmp = new TH2D(dumChar,dumChar,700,0,7,200,-100*enCal,100*enCal);
	...
	for (int i = minStdDev; i < nStdDev; ++i)
	{
		...
		noMaskLvEn->Draw("Colz");
		fragCut[i]->Draw("same f");
		fragCut[i]->Draw("same");
		alphaCut[i]->Draw("same f");
		alphaCut[i]->Draw("same");
