...
plotdir=`pwd`/vertexCft.${actinide}.${preamp}preamp.$pidType.pidCut$pidCut.`date +%d%b%y_%H%M.%S`
mkdir $plotdir
cd $plotdir
cp $macros/vertexCft.{sh,C} $plotdir

echo root -l -q -b vertexCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\",\"$driftVel\"\)
time root -l -q -b vertexCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\",\"$driftVel\"\)
wait
