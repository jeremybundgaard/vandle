...
void buildAlphaRidgeCut1stPreampHK() {

TFile* noMaskFile = new TFile("$cftrees/hk.cfNoMask.1stPreamp.trees.root");
...
noMaskTracks->Draw("length:adc*168.6/40000>>noMaskLvEn","fitType==5","colz");
Double_t xCut[38];
Double_t yCut[38];
char projname[123];

int arrayPos=0;
for (int lbinStep = 100; lbinStep < 300; lbinStep+=5){
	...
	std::cout << "xCut , yCut"<<'\t'<<xCut[arrayPos] <<" , "<<yCut[arrayPos] <<'\t'<<"arrayPos="<<arrayPos<< std::endl;
	arrayPos++;
}

c1->SetLogy(0);
TGraph *g = new TGraph((sizeof(xCut) / sizeof(Double_t)), xCut, yCut);
TF1 *f = new TF1("f", "[3] + [2]*TMath::Exp( [1]*x + [0] )",1,xCut[38 - 1]);
f->SetLineWidth(3);
f->SetLineColor(kRed);
g->Fit(f);
noMaskLvEn->SetTitle(f->GetTitle());
noMaskLvEn->Draw("Colz");
f->DrawF1(1,xCut[38 - 1],"same");
