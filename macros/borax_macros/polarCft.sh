...
plotdir=`pwd`/polarCft.${actinide}.${preamp}preamp.$pidType.pidCut$pidCut.`date +%d%b%y_%H%M.%S`
mkdir $plotdir
cd $plotdir
cp $macros/polarCft.{sh,C} $plotdir

echo root -l -q -b polarCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\",\"$driftVel\"\)
time root -l -q -b polarCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\",\"$driftVel\"\)
wait
mkdir backups pngs grid_plots
mv polarCft.{sh,C} backups/.
