...
for (size_t cut_qdc = minQDC; cut_qdc <= maxQDC; cut_qdc+=cutStep_qdc) {
               ...
               const char* cutChar_local)
{
  ...
  c1->SetLogz(1);
  TH2D* QDCvsTOF = new TH2D("QDCvsTOF","QDCvsTOF",300,-30,970,maxQDC*1.3,0,maxQDC*1.3);
  ...
  for (int i_cut = tcutg_BACKGROUND_local.size()-1; i_cut >= 0; i_cut--) {
    ...
    TH1D* backgrounds = new TH1D("backgrounds","backgrounds",1000,-30,970);
    ...
    sprintf(TOF_background_char,"%s_TOF_background_%d.png",isotope_local,i_cut);
    ...
    sprintf(QDCvsTOF_background_char,"%s_QDCvsTOF_background_%d.png",isotope_local,i_cut);
    c1->SaveAs(QDCvsTOF_background_char);

    sprintf(buffChar_local,
      "gm montage -mode concatenate -tile 2x1  %s %s ../plots/%s_background_projections_%d.png",
